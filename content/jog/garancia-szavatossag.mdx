---
title: "Garancia és szavatosság az építkezésnél – mit követelhetsz, ha baj van?"
description: "Építkezési garancia: 3, 5 és 10 éves jótállási idők, szavatossági szabályok, reklamáció menete. Ptk. és 181/2003 rendelet alapján, konkrét példákkal."
canonical: "https://hazepitesikalauz.hu/jog/garancia-szavatossag"
published_at: "2026-03-25"
refreshed_at: "2026-03-25"
articleSchema:
  "@context": "https://schema.org"
  "@type": "Article"
  "@id": "https://hazepitesikalauz.hu/jog/garancia-szavatossag"
  "headline": "Garancia és szavatosság az építkezésnél – mit követelhetsz, ha baj van?"
  "description": "Építkezési garancia: 3, 5 és 10 éves jótállási idők, szavatossági szabályok, reklamáció menete."
  "image":
    "@type": "ImageObject"
    "url": "https://hazepitesikalauz.hu/img/docs/jog/garancia-szavatossag-hero.svg"
    "width": 1200
    "height": 630
  "datePublished": "2026-03-25"
  "dateModified": "2026-03-25"
  "inLanguage": "hu"
  "author":
    "@type": "Organization"
    "name": "Házépítési Kalauz"
    "url": "https://hazepitesikalauz.hu"
  "publisher":
    "@type": "Organization"
    "name": "Házépítési Kalauz"
    "url": "https://hazepitesikalauz.hu"
  "mainEntityOfPage":
    "@type": "WebPage"
    "@id": "https://hazepitesikalauz.hu/jog/garancia-szavatossag"
faqPageSchema:
  "@context": "https://schema.org"
  "@type": "FAQPage"
  "question1": "Mi a különbség a jótállás és a szavatosság között?"
  "answer1": "A jótállás (garancia) jogszabályon vagy szerződésen alapul, és a kivitelezőt terheli a bizonyítás, hogy a hiba nem az ő felelőssége. A szavatossággal szemben az első 6 hónapban automatikusan a kivitelezőnek kell bizonyítania."
  "question2": "Mennyi a kötelező jótállási idő új építésű háznál?"
  "answer2": "A 181/2003 rendelet az épületelem típusa szerint határozza meg: 3 év (falszerkezet, burkolat, gépészet), 5 év (tető, nyílászárók, klíma), 10 év (teherhordó szerkezet, alapozás, vízszigetelés)."
  "question3": "Hogyan kell reklamálni építési hibát?"
  "answer3": "Írásban, ajánlott levélben vagy e-mailben, a hiba pontos leírásával és fényképekkel. A kivitelezőnek 15 napon belül érdemben válaszolnia kell. Ha nem javít, fogyasztóvédelmi hatósághoz vagy bírósághoz fordulhatsz."
  "question4": "Mit tehetek ha a kivitelező nem javítja a garanciális hibát?"
  "answer4": "Először írásos felszólítás, majd békéltető testületi eljárás vagy polgári per indítható. Ha a hiba veszélyes (pl. statikai), az építésügyi hatóság is bevonható. A per költsége 50-300 ezer Ft, az eljárás 1-3 év."
  "question5": "A tervező is felelős az építési hibákért?"
  "answer5": "Igen, ha a hiba tervezési eredetűre vezethető vissza. A tervező felelőssége a Ptk. szerint 5 éves elévülési időn belül érvényesíthető, és 2025 januárjától kötelező felelősségbiztosítással kell rendelkeznie."
---

import InfoCard from '../../components/mdx/InfoCard.tsx'
import CaseStudy from '../../components/mdx/CaseStudy.tsx'
import ExpertQuote from '../../components/mdx/ExpertQuote.tsx'
import ComparisonRow from '../../components/mdx/ComparisonRow.tsx'

## Miért számít ez neked mint építtetőnek?

A műszaki átadás napján a házad tökéletesnek tűnik. Fél év múlva megjelenik az első repedés a falon. Egy év múlva beázik a tetőablak. Három év múlva kiderül, hogy a padlófűtés csöve szivárog a nappali alatt.

A kérdés nem az, hogy lesz-e probléma. A kérdés az, hogy ki fizeti a javítást. Ez a cikk arról szól, mire van törvényes jogod – és hogyan érvényesítsd.

## Jótállás vs. szavatosság – a két fogalom nem ugyanaz

A legtöbb építtető szinonimaként használja a „garancia" és „szavatosság" szavakat. Nem azok.

<ComparisonRow
  label="Bizonyítási teher"
  left="Kivitelezőé (ő bizonyítja, hogy nem hibás)"
  right="Megrendelőé (te bizonyítod a hibát)"
  leftHeader="Jótállás (garancia)"
  rightHeader="Szavatosság"
  winner="left"
/>

<ComparisonRow
  label="Jogalap"
  left="Jogszabály (181/2003 r.) vagy szerződés"
  right="Ptk. automatikusan"
  leftHeader="Jótállás"
  rightHeader="Szavatosság"
/>

<ComparisonRow
  label="Időtartam"
  left="3/5/10 év (épületelem típusa alapján)"
  right="5 év ingatlanra (Ptk. 6:163.§)"
  leftHeader="Jótállás"
  rightHeader="Szavatosság"
/>

A gyakorlatban a jótállás az erősebb fegyver. Ha jótállási időn belül jelentkezik a hiba, a kivitelezőnek kell bizonyítania, hogy nem az ő munkája okozta. Szavatosságnál fordított: neked kell bizonyítanod.

<InfoCard type="tip" title="A 6 hónapos vélelem">
  A Ptk. 6:163.§ (2) szerint a teljesítéstől számított **6 hónapon belül** felismert hiba esetén vélelmezni kell, hogy az már az átadáskor fennállt – vagyis a bizonyítási teher ilyenkor a szavatosságnál is a kivitelezőé. Ezt a 6 hónapot ne hagyd kárba veszni.
</InfoCard>

## A kötelező jótállási idők – 181/2003. (XI. 5.) Korm. rendelet

A [181/2003. (XI. 5.) Korm. rendelet](https://net.jogtar.hu/jogszabaly?docid=a0300181.kor) az **épületelem típusa** alapján határozza meg a kötelező jótállási időt (a mellékletekben felsorolva):

<ComparisonRow
  label="3 éves jótállás (1-2. melléklet)"
  left="Falszerkezet, belső burkolat, beépített bútorok, épületgépészeti berendezések felszerelése, közös helyiségek (lépházak, garázsok)"
  right="A legrövidebb jótállási kategória"
  leftHeader="Mit fed"
  rightHeader="Megjegyzés"
/>

<ComparisonRow
  label="5 éves jótállás (3. melléklet)"
  left="Tetőhéjalás, nyílászárók (ablakok, ajtók), klíma- és szellőzőberendezések, liftek"
  right="A legtöbb felújítási vitát ez a kategória érinti"
  leftHeader="Mit fed"
  rightHeader="Megjegyzés"
/>

<ComparisonRow
  label="10 éves jótállás (4. melléklet)"
  left="Födém, teherhordó vázszerkezet, alapozás, talajvíz és talajnedvesség elleni szigetelés"
  right="A szerkezeti elemekre a legerősebb védelem"
  leftHeader="Mit fed"
  rightHeader="Megjegyzés"
/>

Vagyis: ha a tetőd beázik, 5 év áll rendelkezésedre. Ha az alapod süllyed, 10 év. Ha a csempe felválik, 3 év. Az időtartam az épületelem típusától függ, nem a szerződés összegétől.

<InfoCard type="warn" title="A jótállás NEM mondható le">
  A kötelező jótállás a fogyasztó javára szóló kógens szabály. A kivitelező nem írhatja a szerződésbe, hogy „a jótállás 1 év" vagy „garanciát nem vállalunk". Az ilyen szerződési kitétel semmis – de sokan nem tudják, és nem érvényesítik a jogukat.
</InfoCard>

## Mire terjed ki a jótállás – és mire nem?

**A jótállás fedezi:**
- Szerkezeti hibákat (repedések, süllyedés, deformáció)
- Vízszigetelési problémákat (beázás, párakicsapódás)
- Hőszigetelési hibákat (hőhidak, kondenzvíz)
- Gépészeti rendszerek meghibásodását (fűtés, víz, csatorna)
- Villanyszerelési hibákat (zárlatok, rossz érintésvédelem)
- Burkolási hibákat (felváló csempe, repedező fugák)

**NEM fedezi:**
- Természetes kopást és elhasználódást
- Megrendelő által okozott kárt (pl. fúrtál a padlófűtés csövébe)
- Vis maior (árvíz, földrengés, villámcsapás)
- Rendeltetésellenes használatot
- Karbantartás elmulasztásából eredő hibákat

<CaseStudy name="Kovács család" location="Érd" year={2024}>
  130 m²-es téglaházukat 2021-ben adták át. 2024 tavaszán a nappali padlója alatt 3 m²-en felpúposodott a laminált padló. A kivitelező azt állította: „párásodás, nem garancia". Független szakértő megállapította: az aljzatbeton nem száradt ki megfelelően az átadás előtt – ez kivitelezési hiba. 5 éves jótállás alatt a kivitelező köteles volt a javítást elvégezni. Költség: 480 000 Ft – amit a kivitelező fizetett.
</CaseStudy>

## A reklamáció menete – hogyan érvényesítsd a jogaidat?

**1. lépés: Dokumentáld a hibát**
- Fotók, videók készítése dátummal
- A hiba pontos leírása (hol, mikor észlelted, milyen körülmények között)
- Ha van korábbi fénykép az adott részről (pl. átadáskor), az aranyat ér

**2. lépés: Írásbeli bejelentés**
- Ajánlott levél VAGY e-mail (tértivevénnyel / olvasási visszaigazolással)
- Hivatkozz a szerződésre és a 181/2003. Korm. rendeletre
- Tűzz ki ésszerű határidőt (15–30 nap)

**3. lépés: A kivitelező válaszol (vagy nem)**
- Ha elismeri: egyeztessetek javítási határidőt
- Ha vitatja: független igazságügyi szakértőt kell bevonni
- Ha nem válaszol 15 napon belül: ez a joggyakorlatban a hiba elismerésének minősülhet

**4. lépés: Ha nem javít**
- Békéltető testületi eljárás (díjmentes, 90 napos határidő)
- Fogyasztóvédelmi hatósághoz fordulás
- Végső esetben: polgári per (illeték: a követelés 6%-a, min. 15 000 Ft)

<InfoCard type="info" title="Igazságügyi szakértő költsége">
  Egy építészeti igazságügyi szakértői vélemény 150 000–500 000 Ft között mozog, a hiba összetettségétől függően. Ez drága – de ha a jótállási igényed megalapozott, a szakértő díját is a kivitelezőre háríthatod.
</InfoCard>

## A tervező felelőssége – nem csak a kivitelező hibázhat

Ha a hiba nem a kivitelezésből, hanem a tervekből ered (pl. aluláretezett alapozás, rossz hőhíd-kezelés, hibás gépészeti terv), a **tervező felelős** a Ptk. vállalkozási szerződésre vonatkozó szabályai szerint.

A tervezői felelősség jellemzői:
- **Elévülési idő:** 5 év az átadástól
- **Bizonyítás:** Független szakértővel kell igazolni, hogy a hiba tervezési eredetű
- **2025. január 15-től:** Kötelező tervezői felelősségbiztosítás – a MÉK évente ellenőrzi

A [tervezői művezetés](/jog/epitesi-engedelyek) (amely 2024. október 1-től kötelező) pont azért lett bevezetve, hogy a tervező személyesen ellenőrizze a helyszínen, hogy a kivitelezés a tervek szerint halad-e. Ha a művezetés során nem jelzett hibát, ami később problémaként jelentkezik, az ő felelősségét is megalapozhatja.

## Amit a kivitelezők nem mondanak el

**A „normál kopás" érv:** A leggyakoribb visszautasítási formula. De a falrepedés az átadás utáni 2 évben NEM normál kopás. A beázó tetőablak NEM normál kopás. A felpúposodó parketta NEM normál kopás. Ezek kivitelezési hibák – és a jótállás fedezi őket.

**A „te módosítottad" érv:** Ha az átadás után nem nyúltál a kérdéses szerkezethez, ez az érv nem áll meg. Az e-építési napló rögzíti a tényleges kivitelezési műveleteket – ez a te bizonyítékod.

**A „mi megcsináltuk volna, de nem engedtél be" érv:** Ha a kivitelező kér időpontot a javításra, add meg. Ha akadályozod a javítást, elveszítheted a jótállási igényedet. De: te határozod meg a számotokra elfogadható időpontot, nem ő.

<ExpertQuote name="Dr. Koncz Kornélia" role="építési jogi ügyvéd" source="drkonczugyvediiroda.hu">
  Az építési hibák miatti perek átlagosan 3-4 évet tartanak, de egyes ügyek 8-10 évig is húzódhatnak. A legjobb védelem a megelőzés: részletes szerződés, független műszaki ellenőr, és az e-napló rendszeres ellenőrzése.
</ExpertQuote>

## Hogyan védd magad szerződéssel?

A [kivitelezési szerződés](/kivitelezes/kivitelezo-valasztas) a legfontosabb jogi eszközöd. Ami nincs benne, azt nehéz érvényesíteni.

**Minimum tartalomjegyzék a garanciális részhez:**
- Jótállási időtartam (hivatkozás a 181/2003. rendeletre)
- Hibabejelentés módja és határideje
- Javítási határidő (max. 30 nap ajánlott)
- Kötbér nem teljesítés esetén (napi 0,1-0,5% a szerződés összegéből)
- Visszatartott összeg (5-10% a végszámláról, jótállási időre letétbe helyezve)
- Alvállalkozók garanciális felelőssége (a fővállalkozó felelős az alvállalkozóiért)

<InfoCard type="tip" title="Az 5-10%-os visszatartás">
  Kösd ki a szerződésben, hogy a végszámla 5-10%-át a jótállási idő végéig (vagy legalább 1 évig) visszatartod. Ez a leghatékonyabb motiváció arra, hogy a kivitelező a garanciális javításokat időben elvégezze. Ha nem megy vissza javítani, a visszatartott összegből más szakemberrel végeztetheted el.
</InfoCard>

## A kötelező felelősségbiztosítás – mi változott 2025-ben?

2025. január 15-től **minden kivitelező és tervező köteles felelősségbiztosítással rendelkezni**. Az adatokat az [e-építési naplóba](https://www.e-epites.hu/e-naplo) kell feltölteni.

Ez azt jelenti, hogy ha a kivitelező csődbe megy vagy megszűnik, a biztosítója fizeti a garanciális javítás költségét – ha az biztosítási eseménynek minősül.

Amit ellenőrizz:
- Kérd el a biztosítási kötvény másolatát a szerződéskötés ELŐTT
- Ellenőrizd, hogy a biztosítás az építkezés teljes időtartamára + a jótállási időre vonatkozik
- Nézd meg a biztosítási összeg felső határát (minimum 10 millió Ft ajánlott)

## Személyre szabott jogi tanácsadás kell?

Minden építkezés egyedi. Ha bizonytalan vagy a garanciális jogaidban, kérj jogi tanácsadást egy építési jogban jártas ügyvédtől – még a szerződéskötés ELŐTT.

[Ingyenes árajánlatot kérek →](/ajanlatkeres)

::: faq
### Mi a különbség a jótállás és a szavatosság között?
A jótállás (garancia) jogszabályon vagy szerződésen alapul, és a kivitelezőt terheli a bizonyítás, hogy a hiba nem az ő felelőssége. A szavatossággal szemben az első 6 hónapban automatikusan a kivitelezőnek kell bizonyítania. A jótállás erősebb védelmet ad.
:::

::: faq
### Mennyi a kötelező jótállási idő új építésű háznál?
A 181/2003 rendelet az épületelem típusa szerint határozza meg: 3 év (falszerkezet, burkolat, gépészet), 5 év (tető, nyílászárók, klíma, lift), 10 év (teherhordó szerkezet, alapozás, vízszigetelés). Tehát ugyanazon a házon belül eltérő jótállási idők vonatkoznak a különböző elemekre.
:::

::: faq
### Hogyan kell reklamálni építési hibát?
Írásban, ajánlott levélben vagy e-mailben, a hiba pontos leírásával és fényképekkel. Hivatkozz a szerződésre és a 181/2003. Korm. rendeletre. A kivitelezőnek 15 napon belül érdemben válaszolnia kell. Ha nem javít, békéltető testülethez vagy bírósághoz fordulhatsz.
:::

::: faq
### Mit tehetek ha a kivitelező nem javítja a garanciális hibát?
Először írásos felszólítás, majd békéltető testületi eljárás (díjmentes) vagy polgári per indítható. Ha a hiba veszélyes (pl. statikai), az építésügyi hatóság is bevonható. Független igazságügyi szakértő véleménye szükséges a perben, ennek díja 150-500 ezer Ft.
:::

::: faq
### A tervező is felelős az építési hibákért?
Igen, ha a hiba tervezési eredetűre vezethető vissza. A tervező felelőssége a Ptk. szerint 5 éves elévülési időn belül érvényesíthető, és 2025 januárjától kötelező felelősségbiztosítással kell rendelkeznie. A tervezői művezetés (2024.10.01-től kötelező) pont ennek megelőzését szolgálja.
:::
